# -*- coding: utf-8 -*-
# ============================================================
#  Muraqqam Challenge — v31 (السكربت النهائي — آخر 24 ساعة)
#  المطلوب: train.csv و test.csv و artifacts.npz (ذخيرة v17)
#
#  يفعل كل المتبقي في تشغيلة واحدة، ويتكيف مع الموارد تلقائيا:
#    1) إن وُجد GPU: يدرّب AG1 (AraBERT مع augmentation — تلميح المنظّم) ~2.5س
#       إن لم يوجد GPU: يتخطى التدريب بأمان ويكمل بالمكونات الموجودة
#    2) دمج كل الخزّان + مكونات ML بطبقة Stacking -> submission_stack.csv
#    3) تصويت نهائي: القديم 0.688 + التصويت السابق + الـ stack الجديد
#       -> submission_final.csv (وإن غابت ملفات التصويت يبقى الـ stack هو النهائي)
#  المدخلات: train/test + artifacts.npz + probs_backup*.pkl
#            + (اختياري للتصويت) ملف القديم 0.688 وملف التصويت السابق 0.686
...
CONF = 0.90
TRAIN_AG1 = True   # AraBERT مع augmentation
TRAIN_ML  = False
NUM_EPOCHS_ML = 1 if FAST_RUN else 6
AUG_SYNTH_FACTOR = 2    # مستندات صناعية = ضعفا عدد مستندات التدريب
AUG_RARE_DUP     = 3    # نوافذ ! و ؛ تتكرر 3 مرات

MODEL_AG1 = 'aubmindlab/bert-base-arabertv02'
MODEL_ML  = 'aubmindlab/bert-base-arabertv02'
...
def make_synth_docs(real_docs, factor, seed):
    """يقطع مستندات التدريب إلى جُمل (عند . ! ؟) ويعيد تركيبها عشوائيا."""
...
def train_family(model_name, n_folds, seed_offset, tag, pseudo,
    ...
    for fold in range(n_folds):
        ...
        tr = tr_real + pseudo
        # مستندات صناعية من جُمل طية التدريب فقط (لا تسريب من طية التحقق)
        ...
        log(f'  نوافذ التدريب: {len(tr_items)} (منها نادرة مكررة: {len(rare_items)}x{AUG_RARE_DUP-1})')
...
ADD = []
if TRAIN_AG1 and torch.cuda.is_available():
    ADD.append(('ag1', MODEL_AG1, 3000, 5, dict(bs=16, lr=3e-5, epochs=8)))
elif TRAIN_AG1:
    log('لا يوجد GPU — تخطي تدريب AG1 والاكتفاء بالمكونات الموجودة')
...
def train_ml_family(model_name, n_folds, seed_offset, tag, pseudo):
    global tokenizer
    tokenizer = AutoTokenizer.from_pretrained(model_name)
    rng = random.Random(BASE_SEED)
    order = list(range(len(docs))); rng.shuffle(order)
    ...
    for fold in range(n_folds):
        ...
        dl = DataLoader(MLDataset(tr_items), batch_size=16, shuffle=True,
...
if TRAIN_ML:
    ml_oof_docs, ml_test_docs = train_ml_family(MODEL_ML, NF, 2000, 'ML1', pseudo_docs)
...
if TRAIN_ML:
    ...
    with open(os.path.join(WORK_DIR, 'probs_backup.pkl'), 'wb') as f:
        pickle.dump({'oof': [a[:, PERM] for a in ml_oof_docs],
                     'test': [a[:, PERM] for a in ml_test_docs]}, f)
    log('حُفظت ذخيرة ML: probs_backup.pkl')
_sync_to_drive()
...
pd.DataFrame({'id': test_df['id'], 'final_text': finals}).to_csv(
    os.path.join(WORK_DIR, 'submission_stack.csv'), index=False)
log('تم حفظ submission_stack.csv')

# ---------- 9) التصويت النهائي: القديم + التصويت السابق + الـ stack الجديد ----------
    ...
    for pat in patterns:
        ...
        for root in ('/kaggle/input', '/content', '.'):
            if os.path.exists(root):
                hits += [h for h in _glob.glob(os.path.join(root, '**', pat), recursive=True)
                         if 'sample_submission' not in h and '/working/' not in h.replace(WORK_DIR, '/working/')]
        hits = sorted(set(hits), key=lambda p: (len(p), p))
        if hits: return hits[0]
    return None

def _to_sets(pred):
    return [set(c for c in g if c in VALID_SYMBOLS) for _, g in _tokenize_gold(str(pred))[1]]

old_path  = _find_csv(['submission_old*.csv', 'submission (18)*.csv', 'submission*18*.csv'])
vote_path = _find_csv(['submission_vote*.csv', '*v28*vote*.csv', 'muraqqam_v28*.csv'])

if old_path and vote_path:
    log(f'التصويت النهائي بين: القديم ({old_path}) + التصويت السابق ({vote_path}) + الـ stack الجديد')
    voters = []
    ok = True
    for p in (old_path, vote_path):
        df = pd.read_csv(p)
        if len(df) != len(test_df):
            log(f'!! {p}: عدد صفوف لا يطابق — إلغاء التصويت'); ok = False; break
        sets_docs = []
        for raw, pred in zip(test_df['text'], df['final_text']):
            s = _to_sets(pred)
            if len(s) != len(str(raw).strip().split()):
                log(f'!! {p}: محاذاة خاطئة — إلغاء التصويت'); ok = False; break
            sets_docs.append(s)
        if not ok: break
        voters.append(sets_docs)
    if ok:
        voters.append([_to_sets(f) for f in finals])   # الـ stack الجديد
        ORDER_V = ['؟', '!', '.', ':', '؛', '،', '-']
        TERM_V = set('.!؟')
        finals2 = []
        for ti, ws in enumerate(test_words_all):
            ...
            for wi in range(len(ws)):
                votes = [voters[k][ti][wi] for k in range(3)]
                chosen = set(s for s in SYMBOLS if sum(s in v for v in votes) >= 2)
                if wi == len(ws) - 1 and not (chosen & TERM_V):
                    chosen.add('.')
                parts.append(ws[wi] + ''.join(s for s in ORDER_V if s in chosen))
            finals2.append(' '.join(parts))
        bad2 = 0
        for raw, fin in zip(test_df['text'], finals2):
            try:
                _extract_labels(raw, fin, 'prediction')
            except ValueError:
                bad2 += 1
        if bad2 == 0:
            pd.DataFrame({'id': test_df['id'], 'final_text': finals2}).to_csv(
                os.path.join(WORK_DIR, 'submission_final.csv'), index=False)
            log('تم حفظ submission_final.csv (التصويت النهائي) — هذا هو المرشح الأقوى للتسليم')
        else:
            log(f'!! فشل فحص التصويت النهائي ({bad2} صفوف) — استخدمي submission_stack.csv')
else:
    log('ملفا التصويت (القديم/السابق) غير موجودين — submission_stack.csv هو الملف النهائي')

log('=== الخلاصة ===')
log('1) submission_final.csv (إن وُجد) = تصويت [القديم 0.688 + تصويت 0.686 + stack جديد] -> سلّميه')
log('2) submission_stack.csv = الـ stack وحده -> سلّميه أيضا إن بقيت تسليمات للمقارنة')
log('3) النهائيان المختاران (Select): القديم 0.688 + الأفضل من هذين على اللوحة')
_sync_to_drive()
