# -*- coding: utf-8 -*-
# ============================================================
#  Muraqqam Challenge — v24 (الجولة الأخيرة: طيات أكثر + الكبير بتسميات محسّنة)
#  المطلوب: train.csv و test.csv و artifacts.npz (ذخيرة v17)
#
#  الجولة الأخيرة (~5 ساعات على T4، أسرع على 2xT4):
#    المدخلات: artifacts.npz بسبعة مكونات + probs_backup.pkl (من v23)
#    1) AR5 = AraBERT-base، 8 طيات × 8 حقب (بيانات أكثر لكل نموذج مثل نظامك القديم)
#    2) SL2 = AraBERT-large، 5 طيات × 6 حقب — بتسميات زائفة من الخزّان السباعي
#    3) دمج الكل (9 softmax + ML) بطبقة Stacking -> submission.csv
...
CONF = 0.90
TRAIN_AR5 = True   # AraBERT-base بثماني طيات
TRAIN_SL2 = True   # AraBERT-large بتسميات الخزّان السباعي
TRAIN_ML  = False  # ML1 موجود في probs_backup.pkl — لا حاجة لإعادة تدريبه
NUM_EPOCHS_ML = 1 if FAST_RUN else 6

MODEL_AR5 = 'aubmindlab/bert-base-arabertv02'
MODEL_SL2 = 'aubmindlab/bert-large-arabertv02'
...
ADD = []
if TRAIN_AR5: ADD.append(('ar5', MODEL_AR5, 1500, 8, dict(bs=16, lr=3e-5, epochs=8)))
if TRAIN_SL2: ADD.append(('sl2', MODEL_SL2, 1600, 5, dict(bs=8, lr=2e-5, ga=2, epochs=6)))
