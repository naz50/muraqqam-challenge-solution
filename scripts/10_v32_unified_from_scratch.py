# -*- coding: utf-8 -*-
# ============================================================
#  Muraqqam Challenge — v32 (كود النشر الموحد: النظام كاملا من الصفر)
#  المطلوب: train.csv و test.csv و artifacts.npz (ذخيرة v17)
#
#  سكربت النشر الكامل: يعيد بناء النظام النهائي (Public 0.685 / OOF 0.6945)
#  من الصفر — لا يحتاج إلا train.csv و test.csv.
#
#  المكونات (بالترتيب التاريخي، بذور مثبتة):
#    الجيل 1: ar2 (AraBERT) بلا pseudo -> pseudo -> ca1 (CAMeLBERT) + arL (AraBERT-large)
#    الجيل 2: pseudo -> el1 (AraELECTRA) + ar3 (AraBERT بذرة 2)
#    الجيل 3: pseudo -> ar4 + ca2 (8 حقب) + ML1 (AraBERT برأس multi-label)
#    الجيل 4: pseudo -> ar5 (8 طيات) + sl2 (large بتسميات محسنة)
#    الجيل 5: pseudo -> ca3 + ar6 (8 طيات) + ML2 (large multi-label)
#    الجيل 6: pseudo -> ab1 (ARBERT) + as1 (asafaya)
#    الجيل 7: pseudo -> ag1 (AraBERT + augmentation: جمل معاد تركيبها + تكثيف ! و ؛)
#    ثم: Stacking (لوجستي لكل رمز + ميزات الجيران) -> submission.csv
#
#  الزمن الكلي: ~20-26 ساعة GPU (T4). أكثر من جلسة واحدة:
#    كل طية تُحفظ في كاش فور اكتمالها. لاستئناف عبر جلسات Kaggle:
#    أضيفي مخرجات النسخة السابقة (مجلد foldcache_v17) كمدخل Dataset —
#    السكربت يستورد أي كاش يجده في /kaggle/input تلقائيا ويكمل من حيث توقف.
...
# استيراد كاش من جلسات سابقة (أضيفي مخرجات النسخة السابقة كمدخل)
...
def train_family(model_name, n_folds, seed_offset, tag, pseudo,
    ...
    for fold in range(n_folds):
        ...
        log(f'  نوافذ التدريب: {len(tr_items)}')
...
# ---------- 3) محرك الأجيال (من الصفر — بلا أي ذخيرة خارجية) ----------
reservoir_oof, reservoir_test = {}, {}
pseudo_docs = []

def regen_pseudo():
    """تسميات زائفة من متوسط كل مكونات الخزان الحالي (ثقة >= 0.90)."""
    global pseudo_docs
    ...
    log(f'تسميات زائفة ({len(keys)} مكونا): {kept}/{tot} ({kept/max(tot,1):.1%})')

# الخطوات بالترتيب التاريخي — 'P' = إعادة توليد التسميات الزائفة
...
STEPS = [
    ...
    ('S', 'arL', M_L, 800, 4, dict(bs=8, lr=2e-5, ga=2, epochs=6), False),
    'P',
    ('S', 'el1', M_EL, 500, 4, dict(bs=16, lr=3e-5, epochs=6), False),
    ('S', 'ar3', M_AR, 900, 4, dict(bs=16, lr=3e-5, epochs=6), False),
    'P',
    ('S', 'ar4', M_AR, 1200, 4, dict(bs=16, lr=3e-5, epochs=8), False),
    ('S', 'ca2', M_CA, 1300, 4, dict(bs=16, lr=3e-5, epochs=8), False),
    ('M', 'ML1', M_AR, 2000, 4, dict(bs=16), False),
    'P',
    ('S', 'ar5', M_AR, 1500, 8, dict(bs=16, lr=3e-5, epochs=8), False),
    ('S', 'sl2', M_L, 1600, 5, dict(bs=8, lr=2e-5, ga=2, epochs=6), False),
    'P',
    ('S', 'ca3', M_CA, 1700, 8, dict(bs=16, lr=3e-5, epochs=8), False),
    ('S', 'ar6', M_AR, 1800, 8, dict(bs=16, lr=3e-5, epochs=8), False),
    ('M', 'ML2', M_L, 2100, 5, dict(bs=8), False),
    'P',
    ('S', 'ab1', M_AB, 2300, 8, dict(bs=16, lr=3e-5, epochs=8), False),
    ('S', 'as1', M_AS, 2400, 8, dict(bs=16, lr=3e-5, epochs=8), False),
    'P',
    ('S', 'ag1', M_AR, 3000, 5, dict(bs=16, lr=3e-5, epochs=8), True),
]

ml_outputs = {}   # ML1/ML2 -> (oof_docs, test_docs) بسبعة أعمدة
AUGMENT_STEP = False   # تُضبط لكل خطوة
...
if not torch.cuda.is_available():
    log('!! هذا السكربت تدريب كامل — يحتاج GPU. فعّلي T4 ثم أعيدي.')
    raise SystemExit()
...
for step in STEPS:
    ...
    if kind == 'S':
        ...
        log(f'{key} منفردة: OOF={macro_of_probs(np.concatenate(oof_n, axis=0)):.4f}')
    else:
        o, t = train_ml_family(resolve_model(mname), nf, soff, key, pseudo_docs, **kw)
        ml_outputs[key] = (o, t)
AUGMENT_STEP = False

# ---------- 7ب) حفظ الذخيرة الكاملة ----------
def pack(doc_arrays):
...
log('حُفظت الذخيرة الكاملة: ' + str(list(reservoir_oof)) + ' + ' + str(list(ml_outputs)))

# تجهيز مصفوفات الـ stacking من الذاكرة
...
log('تم حفظ submission.csv')


log('=== اكتمل النشر: submission.csv يعيد إنتاج النظام النهائي ===')
_sync_to_drive()
