# -*- coding: utf-8 -*-
# ============================================================
#  Muraqqam Challenge — v26 (حقنة التنويع: معماريات عربية من مصادر مختلفة)
#  المطلوب: train.csv و test.csv و artifacts.npz (ذخيرة v17)
#
#  حقنة التنويع (~4-5 ساعات على T4):
#    المدخلات: artifacts.npz (11 مكونا من v25) + probs_backup.pkl + probs_backup2.pkl
#    1) AB1 = ARBERT (UBC-NLP) — تدريب مسبق مختلف كليا عن AraBERT، 8 طيات
#    2) AS1 = asafaya/bert-base-arabic — مصدر ثالث مستقل، 8 طيات
#    3) دمج الكل بطبقة Stacking -> submission.csv
...
CONF = 0.90
TRAIN_AB = True    # ARBERT (UBC-NLP)
TRAIN_AS = True    # asafaya/bert-base-arabic
TRAIN_ML  = False  # مكونا ML موجودان في ملفات pkl
...
def resolve_model(cands):
    ...
    for c in cands:
        try:
            _AT.from_pretrained(c)
            return c
        except Exception:
            continue
    raise ValueError('تعذر تحميل أي نموذج من: ' + str(cands))

MODEL_AB = ['UBC-NLP/ARBERTv2', 'UBC-NLP/ARBERT']       # يجرب الأحدث ثم الأقدم
MODEL_AS = ['asafaya/bert-base-arabic']
...
ADD = []
if TRAIN_AB: ADD.append(('ab1', resolve_model(MODEL_AB), 2300, 8, dict(bs=16, lr=3e-5, epochs=8)))
if TRAIN_AS: ADD.append(('as1', resolve_model(MODEL_AS), 2400, 8, dict(bs=16, lr=3e-5, epochs=8)))
